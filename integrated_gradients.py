...
@tf.function
def integrated_gradients(model, baseline, input,  target_class_idx, m_steps=50, method='riemann_trapezoidal', batch_size=32):
    """
    Args:
    ...
    for alpha in tf.range(0, len(alphas), batch_size):
        ...
        gradient_batch = compute_gradients(model=model,path_inputs=interpolated_path_input_batch,target_class_idx=target_class_idx)
...
def plot_img_attributions(model,baseline, img, target_class_idx, m_steps=50, cmap=None, overlay_alpha=0.4):
    """
    Args:
    model(keras.Model): A trained model to generate predictions and inspect.
    baseline(Tensor): A 3D image tensor with the shape 
    ...
    """
    # Attributions
    ig_attributions = integrated_gradients(model=model,baseline=baseline,input=img,target_class_idx=target_class_idx,m_steps=m_steps)
...
def create_baseline(image, mode='Black'):
    
    x_dim, y_dim, _ = image.shape
    shape = (x_dim, y_dim, 1)
    baseline = tf.zeros(shape=shape)
    if(mode=='White'):
        baseline = tf.ones(shape=shape)
    elif(mode=='Random'):
        baseline = tf.random.uniform(shape=shape, minval = 0.0, maxval = 1.0)
